...
camPos = np.array([-9., 0., 9.])
camDir = np.array([ 1., 0.,-1.])
  ...
  for i, s in enumerate(spheres):
    ...
    if np.linalg.norm(dfrts)<s.rd:#o ponto mais proximo esta dentro da esfera?
      ...
      if sphId == -1 and d>0:
        ...
        collision = b
        a = k
  return(sphId,collision)

def sumRays(depth,sphere,position):
  n = 100
  s = sphere
  ...
  if s.m == 0:
    if depth == 1:
      ...
      for i in range(n):
        ...
        sI,hitPos = castRay(position+(normal*0.005),rayPt)
        if sI != -1:
          si = spheres[sI]
          r = r + si.r*((s.r)/255)*((s.r)/255)
          g = g + si.g*((s.g)/255)*((s.g)/255)
          b = b + si.b*((s.b)/255)*((s.b)/255)
        else:
          color = skyColor(rayPt-position)
          r = r + color[0]*((s.r)/255)*((s.r)/255)
          g = g + color[1]*((s.g)/255)*((s.g)/255)
          b = b + color[2]*((s.b)/255)*((s.b)/255)
      r = int(r/n)
      g = int(g/n)
      b = int(b/n)
      return([r,g,b])
...
for Y in range(size):
  ...
  for x in range(size):
    ...
    sI,hitPos = castRay(camPos,pxPos)
    if sI != -1:
      #depth = 1
      s = spheres[sI]
      #canvas[Y,x]=[s.r,s.g,s.b]
      canvas[Y,x]=sumRays(1,s,hitPos)
    else:
...
plt.imsave('lovelyCanvas3.png', canvas)